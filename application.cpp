...
void Application :: renderAddNewVehicleMenu() const{
	...
	void Application :: renderAddNewTripMenu() const{
		...
		if(freeVehicles.size()==0){
			this->showDialog("No vehicles are free in give ndate range");
			return;
		}
	...
	void Application :: renderCompleteTripMenu() const{
		...
		try{
			auto trip =this->db->getTripRef()->getRecordId(tripId);
			...
			stringstream ss;
			ss << "Total Fare : " << fare ;
			showDialog("Trip completed successfuly", ss.str());
		}
		catch(Error e){
			this->showDialog(e.getMessage());
		}
